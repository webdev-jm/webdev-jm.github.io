            ...
            <div class="row">
                ...
                <div class="col-lg-6 text-right">
                    @if(auth()->user()->hasRole('superadmin'))
                        <a href="{{ route('user.trash') }}" class="btn btn-outline-danger btn-sm mr-2">
                            ...
                            @foreach($users as $user)
                                <tr>
                                    ...
                                    <td class="align-middle text-left p-0">
                                        <img src="{{$user->adminlte_image()}}" alt="{{$user->name}}" class="user-img m-1" style="width:40px;height:40px;object-fit:cover;border-radius:50%;">
                                        {{$user->name}}
                                    </td>
                                    ...
                                    <td class="align-middle text-right p-0 pr-1">
                                        @if(auth()->user()->can('user impersonate') && auth()->id() !== $user->id && !session()->has('impersonate_original_id'))
                                            <a href="{{ route('impersonate.start', encrypt($user->id)) }}" class="btn btn-sm btn-outline-warning mx-1" title="Impersonate User" onclick="return confirm('Are you sure you want to log in as {{ $user->name }}?');">
                                                <i class="fas fa-user-secret"></i>
                                            </a>
                                        @endif
                                        <a href="{{route('user.show', encrypt($user->id))}}" class="btn btn-info btn-xs mb-0 ml-0">
                                            <i class="fa fa-list"></i>
                                            {{__('adminlte::utilities.view')}}
                                        </a>
                                        @can('user edit')
                                            <a href="{{route('user.edit', encrypt($user->id))}}" class="btn btn-success btn-xs mb-0 ml-0">
